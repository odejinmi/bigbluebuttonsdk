...
class Websocketresponse {
    ...
    if (json["collection"] != null) {
      switch (json["collection"]) {
        ...
        case "slides":
          print(jsonEncode(json));
          if (json["msg"] == "added") {
            if (websocket.slides.isNotEmpty &&
                json["fields"]["presentationId"] !=
                    websocket.slides[0]["fields"]["presentationId"]) {
  ...
  void handleVideoControl(Map<String, dynamic> json) {
    print(json);

    if (json["msg"] == "added") {
      final participants = websocket.participant
          .where(
            (v) => v.fields?.intId == json["fields"]["userId"],
          )
          .toList();

      if (participants.isNotEmpty) {
        final participant = participants[0];
        participant.isvidieo = true;
        participant.vidieoid = json["id"];
        participant.vidieodeviceId = json["fields"]["stream"];
      ...
      final participants = websocket.participant
          .where(
            (v) => v.vidieoid == json["id"],
          )
          .toList();

      if (participants.isNotEmpty) {
        final participant = participants[0];
        participant.isvidieo = false;
        participant.vidieoid = null;
        participant.vidieodeviceId = null;
